...
export async function optimize(
    ...
    // Apply ENS policy constraints
    const effectiveMaxChunks = Math.min(maxChunks, policy.maxChunks ?? MAX_CHUNKS_NORMAL)
    logger.log(`  Max chunks (policy-limited): ${effectiveMaxChunks}`)

    // Filter chains to user's preferred chains if set
    const policyChains = policy.preferredChains ?? []
    if (policyChains.length > 0) {
        logger.log(`  Preferred chains (ENS): ${policyChains.join(", ")}`)
    }
    const ethChain = marketData.chains.find(c => c.chain === "ethereum" && c.available)
    if (!ethChain) {
    ...
    logger.log("")
    logger.log(`  Searching over privateRatio=[0.0, 0.1, ..., 1.0] × nPublic=[0..${effectiveMaxChunks}]`)
    logger.log("")
